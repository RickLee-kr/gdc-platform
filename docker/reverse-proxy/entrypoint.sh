...
# Ensure the development reverse proxy always has a deterministic self-signed PEM pair.
# Admin HTTPS can replace these files later; the bootstrap listener still has valid TLS material.
...
if [ ! -f /etc/nginx/conf.d/default.conf ]; then
  ...
  _need_bootstrap=1
elif ! grep -Eq 'listen[[:space:]]+443[[:space:]]+ssl' /etc/nginx/conf.d/default.conf 2>/dev/null; then
  _need_bootstrap=1
fi
