...
#   - DB name must be gdc or gdc_e2e_test (test-only).
...
#     fixtures without overwriting user-created entities in gdc.
set -euo pipefail
...
Brings up the full E2E lab:
  - PostgreSQL platform catalog (gdc on the dev-validation PostgreSQL port)
...
Environment overrides:
  TEST_DATABASE_URL              gdc on the dev-validation port, or gdc_e2e_test on 127.0.0.1:55432
...
allowed_db_names = {"gdc", "gdc_e2e_test"}
errors: list[str] = []
...
allowed_ports = {5432, 55432, 55442} if db_name == "gdc" else {55432}
if port not in allowed_ports:
    errors.append(f"DATABASE_URL port must be one of {sorted(allowed_ports)} (got {port!r})")
if user != "gdc":
...
if errors:
    ...
    print("  the isolated test profile (loopback, user gdc, db gdc/gdc_e2e_test).", file=sys.stderr)
    sys.exit(1)
...
for _ in $(seq 1 90); do
  if docker compose -p "$COMPOSE_PROJECT" -f "$COMPOSE_FILE" --profile "$PROFILE" exec -T postgres-test \
    pg_isready -U gdc -d gdc >/dev/null 2>&1; then
    break
  fi
  sleep 1
done
