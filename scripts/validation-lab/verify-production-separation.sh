...
step "4) reset-db.sh — refuses non-gdc dev-validation DATABASE_URL"
if [[ ! -x "$RESET_DB" ]]; then
  ...
  if DATABASE_URL="postgresql://produser:prodpass@prod-db.internal.example.com:5432/prod_main" \
    ...
    if grep -qE 'database name must be exactly .?gdc.?' "$TMP_OUT" \
  ...
  if DATABASE_URL="postgresql://gdc:gdc@10.0.0.5:5432/gdc" \
    "$RESET_DB" </dev/null >"$TMP_OUT" 2>&1; then
    bad "reset-db.sh did NOT refuse a non-loopback host"
    sed 's/^/         /' "$TMP_OUT"
  else
    ok "reset-db.sh refused non-loopback host (gdc on remote)"
  fi
  rm -f "$TMP_OUT"
  trap - EXIT
fi
...
if [[ ! -f "$START_DEV" ]]; then
  ...
  if [[ -z "$GATE" ]]; then
    ...
    OUT="$(DATABASE_URL="postgresql://gdc:gdc@127.0.0.1:55442/gdc" APP_ENV="production" \
      python3 "$TMP_GATE" 2>&1)"
    ...
    OUT="$(DATABASE_URL="postgresql://gdc:gdc@127.0.0.1:55442/gdc" APP_ENV="development" \
      python3 "$TMP_GATE" 2>&1)"
