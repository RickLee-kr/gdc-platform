...
# isolated lab Postgres on 127.0.0.1:55442/gdc and never touches production.
...
export TEST_DATABASE_URL="postgresql://gdc:gdc@127.0.0.1:${GDC_DEV_VALIDATION_POSTGRES_HOST_PORT:-55442}/gdc"
...
# Non-zero exit. If alembic upgrade failed (most common cause is gdc having
...
if [[ "$SCHEMA_DRIFT" -eq 1 ]]; then
  cat >&2 <<EOF
  Detected schema drift on the isolated lab database (gdc).
  ...
  Run this exactly (resets ONLY gdc on 127.0.0.1:${GDC_DEV_VALIDATION_POSTGRES_HOST_PORT:-55442}, requires
  typing 'RESET GDC DEV DB' to confirm):
...
cat >&2 <<EOF
  ...
  schema drift was detected — that command erases the dev-validation gdc DB.
EOF
exit $EC
