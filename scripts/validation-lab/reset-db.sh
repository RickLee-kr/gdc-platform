...
#     (host 127.0.0.1/localhost/::1, port 55432, user gdc, database gdc_test).
...
#   - Requires interactive confirmation: type 'RESET GDC TEST DB'.
#   - Drops the public schema in gdc_test and re-runs `alembic upgrade head`.
...
# Run it only when start.sh explicitly reports schema drift on gdc_test.
...
if [[ "$EC" -eq 0 ]]; then
  echo ""
  echo "  gdc_test reset complete. Bring the lab back with:"
  echo "    $START_SELF"
