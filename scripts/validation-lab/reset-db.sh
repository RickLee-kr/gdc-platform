...
#     (host 127.0.0.1/localhost/::1, dev-validation port, user gdc, database gdc).
...
#   - Requires interactive confirmation: type 'RESET GDC DEV DB'.
#   - Drops the public schema in gdc and re-runs `alembic upgrade head`.
...
# Run it only when start.sh explicitly reports schema drift on gdc.
...
if [[ "$EC" -eq 0 ]]; then
  echo ""
  echo "  gdc dev-validation DB reset complete. Bring the lab back with:"
  echo "    $START_SELF"
