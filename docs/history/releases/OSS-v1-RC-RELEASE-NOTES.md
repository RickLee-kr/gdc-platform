...
**Current default:** `GDC_ROUTE_PROCESSING_ENABLED=true` (rollback `false`). SMTP operational email delivery is implemented (`SmtpEmailSender`; default `SMTP_ENABLED=false` skips send). Slack remains planned. See [`KNOWN-LIMITATIONS.md`](../../release/KNOWN-LIMITATIONS.md) and [`source-of-truth-index.md`](../../architecture/source-of-truth-index.md).
