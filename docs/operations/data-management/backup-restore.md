...
Under **Admin Settings → Maintenance Center**, use **Backup & Restore Runbook** to jump to hosted documentation. Operators without a hosted URL should open this file in the repository: `docs/operations/data-management/backup-restore.md`.
...
## Compose release scripts (platform stack)

When using the platform Compose stack, operator scripts under `scripts/release/` are also available:
...
Requirements: `docker` / `docker compose`, healthy `postgres` service, and `GDC_RELEASE_COMPOSE_FILE` aligned with the stack (default `docker-compose.platform.yml`).

- Default output directory: `deploy/backups/` (override with `GDC_BACKUP_DIR`; must resolve under the repository root).
- Filenames: `gdc_pg_<UTC-timestamp>.sql.gz` plus a small `backup_<timestamp>.log`.
- Path safety: refuses backup roots outside the repository checkout and obvious volatile roots such as `/tmp`.
- Scripts never run `docker compose down -v` or `docker volume rm`.

Restore requires `RESTORE_CONFIRM=YES_I_UNDERSTAND` and an interactive confirmation of the exact database name. Unknown DB names other than `gdc` / `datarelay` are refused.
...
After restore: run `alembic upgrade head` if needed, then verify `/health` and Maintenance Center.

Legacy catalog notes and JSON config export/import remain separate — see `specs/015-backup-export-import/spec.md` and `docs/operations/deployment/install-guide.md`. Historical Compose-only draft: `docs/history/implementation-reports/deployment-backup-restore-compose.md`.
