...
자세한 번들 파일 목록은 [`support-bundle.md`](./support-bundle.md)를 참고합니다.
...
| 세션 동작 점검 | `docs/operations/administration/auth-session-operations.md`, `GET /api/v1/auth/whoami` 등 | 운영 절차 문서 |
...
4. 인증 이슈 suspected 시 `docs/operations/administration/auth-session-operations.md` 절차 및 필요 시 `GET /api/v1/auth/whoami` 결과(토큰 값은 마스킹해 공유).
