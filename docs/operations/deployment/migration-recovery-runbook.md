...
2. Restore Postgres from `deploy/backups/` per `docs/operations/data-management/backup-restore.md`  
3. Checkout previous release image/git tag  
4. `alembic upgrade head` only after `validate-migrations` passes on restored DB  

## Related

- `docs/operations/data-management/retention-policies.md` — row deletes on `delivery_logs` (not partition DROP)  
