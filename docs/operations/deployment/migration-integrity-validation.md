...
**Related:** `docs/operations/deployment/migration-recovery-runbook.md` (orphan revisions, `validate-migrations`, backups).
...
See also: `docs/operations/deployment/install-guide.md` (bootstrap admin), `app/db/seed.py` module docstring.
...
Apply the migration chain to the **same** database URL the API uses (see Compose `DATABASE_URL` and `docs/operations/deployment/migration-recovery-runbook.md` for catalog naming).
...
- **Fix:** Follow **`docs/operations/deployment/migration-recovery-runbook.md` § Orphan revision** (backup, inspect schema, validate, stamp or restore chain — never `down -v` / truncate / `git reset` as first resort).
