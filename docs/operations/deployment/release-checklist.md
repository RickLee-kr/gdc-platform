...
- `docs/operations/data-management/backup-restore.md` — PostgreSQL 백업/복원 (ops + Compose)
- `docs/operations/deployment/migration-recovery-runbook.md` — 고아 리비전, `alembic stamp`, 복구
- `docs/operations/deployment/migration-integrity-validation.md` — `migration_integrity`, API 재시작, 시드
- `docs/operations/administration/auth-session-operations.md` — JWT, `token_version`, 로그인 검증
- `docs/testing/backend-full-test.md` — 호스트 `pytest`와 **`gdc_pytest`** 분리
- `docs/release/production-checklist.md` — 현재 production go-live 체크리스트
...
| 카탈로그 이름 | `docker-compose.platform.yml` → **`gdc`**. `deploy/docker-compose.https.yml` → 일반적으로 **`gdc`** (`migration-recovery-runbook.md` 표 참고) |
...
- [ ] **지원 번들** (운영 증빙·이관용)  
  `GET /api/v1/admin/support-bundle` — `docs/operations/troubleshooting/support-bundle.md` 참고.
- [ ] (선택) **런타임/스타트업 로그**  
...
- [ ] `docs/operations/deployment/migration-recovery-runbook.md` 절차(백업 → SQL 검사 → 스키마 일치 시에만 `stamp` 검토 등).
...
| 항목 | 내용 |
|------|------|
| **백업 대상 DB 불일치** | 과거에는 플랫폼 compose와 스크립트 기본값이 어긋날 수 있었으나, **현재는 compose에서 추론**합니다. 커스텀 compose에서는 `POSTGRES_DB`를 확인하고 필요 시 `GDC_BACKUP_DB_NAME`을 명시하세요. |
...
## 10. Release candidate (build / promote) checklist

Use this list before tagging a release candidate or promoting a build to staging/production. Historical English RC draft: `docs/history/releases/deployment-release-checklist-rc.md`.

### Build and tests

- [ ] Full backend CI entrypoint: `GDC_BACKEND_FULL_TEST_RESET_CONFIRM=YES_I_RESET_GDC_TEST_ONLY bash scripts/test/run-backend-full.sh --fresh-schema` (see `docs/testing/backend-full-test.md`).
- [ ] `cd frontend && npm ci && npm test -- --run && npm run build`.
- [ ] `docker compose config -q`, `docker compose -f docker-compose.platform.yml config -q`, and `docker compose -f deploy/docker-compose.https.yml config -q` succeed.
- [ ] Optional RC hardening: `docs/testing/full-e2e-dev-validation.md` — **not** required on every PR.

### Configuration and secrets

- [ ] `.env` from `.env.example`; **no real secrets** in git.
- [ ] `JWT_SECRET_KEY`, `SECRET_KEY`, `ENCRYPTION_KEY`, `GDC_PROXY_RELOAD_TOKEN`, and `POSTGRES_PASSWORD` are strong and unique per environment.
- [ ] `REQUIRE_AUTH=true` for externally reachable deployments; `ENABLE_DEV_VALIDATION_*` flags false on production-facing hosts.

### TLS / database / smoke

- [ ] PEM material under `deploy/tls/` when using HTTPS compose (`docs/operations/deployment/https-reverse-proxy.md`).
- [ ] Alembic at `head`; backup policy in place (`docs/operations/data-management/backup-restore.md`).
- [ ] Post-deploy: `/health`, authenticated login + RBAC spot-check, last known-good image tag recorded.

---

*문서 버전: 저장소 상태 기준 2026-05-16 운영 감사 반영; Phase 2B path consolidation 2026-08-25.*
