...
For scripted install, upgrade, and backup workflows (release candidate), see `docs/operations/deployment/install-guide.md`, `docs/operations/deployment/upgrade-guide.md`, and `docs/operations/data-management/backup-restore.md`.
...
Align nginx `proxy_read_timeout` / `proxy_send_timeout` (and any load balancer idle timeout) with Gunicorn/Uvicorn worker timeouts and bounded DB `statement_timeout` values so clients receive deterministic errors instead of hung sockets. See **`docs/operations/deployment/uvicorn-gunicorn-production.md`**.
