...
**Scope:** Production platform compose does **not** attach to dev-validation external networks or legacy test volumes. WireMock, fixtures, and project `gdc-platform-test` live in `docker-compose.dev-validation.yml` / `docker-compose.test.yml`. See **`docs/testing/dev-validation-lab.md`** and **`docs/development/local-docker-workflow.md`**.
...
See **`docs/operations/deployment/install-guide.md`** for Docker auto-install, ports, and legacy migration.
...
- Self-signed TLS: Admin → HTTPS / Security; see `docs/operations/deployment/https-reverse-proxy.md`.
...
- Lab vs platform: use **`docs/development/local-docker-workflow.md`** when mixing dev-validation fixtures with the platform API.
