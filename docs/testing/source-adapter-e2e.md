...
| --- | --- | --- |
| `postgres-test` | PostgreSQL server: **`gdc`** (API/lab) + **`gdc_pytest`** (host pytest) | `127.0.0.1:55441` |
...
| --- | --- | --- |
| `TEST_DATABASE_URL` | `postgresql://gdc:gdc@127.0.0.1:55441/gdc_pytest` | **Pytest-only** platform catalog (never `gdc` while the API uses it) |
