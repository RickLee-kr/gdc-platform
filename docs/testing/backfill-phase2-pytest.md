...
Set as `TEST_DATABASE_URL` and/or `DATABASE_URL` when running migrations or pytest against the test stack. Do **not** use `gdc` for host pytest — that catalog is for the running API / validation lab.
...
2. Drops and recreates the `public` schema on that database so `alembic upgrade head` is reliable after metadata-only pytest runs (use **only** on **`gdc_pytest`** or `gdc_e2e_test`; do not point at `gdc` or production catalogs).
