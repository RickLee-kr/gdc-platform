...
- Use the **isolated test stack** (`docker-compose.test.yml`) for WireMock E2E so the developer platform PostgreSQL is never targeted by `TEST_DATABASE_URL`. Host pytest uses **`127.0.0.1:55441/gdc_pytest`** (see `tests/db_test_policy.py`); **`gdc`** on that server is for the API / validation lab.
