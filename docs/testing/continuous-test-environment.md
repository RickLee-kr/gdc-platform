...
| **Recommended interval (shared dev server)** | **600–900s** (10–15 min) or nightly smoke via CI, to avoid constant contention on shared dev-validation ports and shared `gdc` lab data. |
...
5. Verify PostgreSQL: `pg_isready -h 127.0.0.1 -p 55442 -U gdc -d gdc`.
