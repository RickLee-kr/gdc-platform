/**
 * Cross-worker Lab API throttle for parallel XP resume.
 *
...
const DEFAULT_LOCK =
  process.env.GDC_XP_CONNECTOR_CREATE_LOCK ||
  path.join('/tmp', 'gdc-xp-connector-create.lock')
...
async function acquireLockFile(lockPath: string, timeoutMs: number): Promise<() => void> {
  ...
  // Stale lock recovery: if holder PID is dead, remove.
  while (true) {
    try {
