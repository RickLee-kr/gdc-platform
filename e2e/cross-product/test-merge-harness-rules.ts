...
// 5) .bad-* quarantine excluded from merge
const runBad = path.join(tmp, 'run-bad')
writeJsonl(path.join(runBad, '.bad-xp-normal-004-ROUTE_ON/cross-product-results.jsonl'), [
  {
    ...base,
    combination_id: 'xp_bad',
    status: 'PASS',
    harness_version: harnessB,
    finishedAt: '2026-07-18T00:00:00Z',
  },
])
writeJsonl(path.join(runBad, 'xp-normal-001-ROUTE_ON/cross-product-results.jsonl'), [
  {
    ...base,
    combination_id: 'xp_good',
    status: 'PASS',
    harness_version: harnessA,
    finishedAt: '2026-07-18T00:00:00Z',
  },
])
const outBad = path.join(tmp, 'out-bad.jsonl')
const mBad = mergeCrossProductResults({ from: runBad, out: outBad })
assert(mBad.written === 1, '.bad-* quarantine excluded from merge')
assert(fs.readFileSync(outBad, 'utf-8').includes('xp_good'), 'kept non-bad shard row')
assert(!fs.readFileSync(outBad, 'utf-8').includes('xp_bad'), 'excluded .bad-* row')

// 6) replacement dir replaces original for same combination_id when only replacement present
const runRep = path.join(tmp, 'run-rep')
fs.mkdirSync(path.join(runRep, 'xp-normal-000-ROUTE_ON'), { recursive: true })
fs.writeFileSync(
  path.join(runRep, 'xp-normal-000-ROUTE_ON/superseded.json'),
  JSON.stringify({ status: 'SUPERSEDED', excluded_from_final_merge: true }),
)
writeJsonl(path.join(runRep, 'xp-normal-000-ROUTE_ON/original/cross-product-results.jsonl'), [
  {
    ...base,
    combination_id: 'xp_s0',
    status: 'FAIL',
    harness_version: harnessB,
    finishedAt: '2026-07-17T00:00:00Z',
  },
])
writeJsonl(path.join(runRep, 'replacements/xp-normal-000-ROUTE_ON/cross-product-results.jsonl'), [
  {
    ...base,
    combination_id: 'xp_s0',
    status: 'PASS',
...
const outRep = path.join(tmp, 'out-rep.jsonl')
const mRep = mergeCrossProductResults({ from: runRep, out: outRep })
assert(mRep.ok, 'replacement-only merge ok')
assert(fs.readFileSync(outRep, 'utf-8').includes('"status":"PASS"'), 'replacement PASS kept')
assert(!fs.readFileSync(outRep, 'utf-8').includes('harnessB') && !fs.readFileSync(outRep, 'utf-8').includes(harnessB), 'superseded original harness excluded')
