...
REPORTS_ROOT="$(python3 -c "from pathlib import Path;import os;print(Path(os.path.expanduser('$REPORTS_ROOT')).resolve())")"
RESULTS="$REPORTS_ROOT/$SOURCE_RUN_ID"
STATE="$RESULTS/recovery-orchestrator-state.json"
LOG="$RESULTS/recovery-orchestrator.log"
ORCH_PID_FILE="$REPORTS_ROOT/.pids/xp_full_on.pid"
RECOVERY_PID_FILE="$REPORTS_ROOT/.pids/xp_recovery.pid"
...
mkdir -p "$RESULTS" "$LOCK_DIR" "$REPORTS_ROOT/.pids"
cd "$E2E"
...
acquire_lock() {
  local my_pid=$$
  if [[ -f "$LOCK_FILE" ]]; then
    # Stale only when PID dead OR process identity mismatch (PID reuse).
    local lock_status
    lock_status="$(
      python3 - <<PY
import sys
...
from recovery_lib import classify_lock
info = classify_lock(Path("$LOCK_FILE"))
print(info["lock_status"])
if info["lock_status"] == "HELD_ACTIVE":
    print(info["lock"].get("pid", ""), file=sys.stderr)
PY
    )"
    ...
    local archive="$LOCK_FILE.stale-$(date -u +%Y%m%dT%H%M%SZ)"
    echo "Archiving stale/mismatched lock ($lock_status) -> $archive"
    mv -f "$LOCK_FILE" "$archive"
  fi
...
from pathlib import Path
sys.path.insert(0, "$E2E/cross-product")
from recovery_lib import build_lock_doc
import json
print(json.dumps(build_lock_doc("$SOURCE_RUN_ID", $my_pid, "$LOCK_FILE"), indent=2))
PY
  ); then
...
# Never overwrite a pinned expected-fixed-harness.json (immutable for this run).
if [[ ! -f "$EXPECTED_HARNESS_FILE" ]]; then
...
if [[ "$EXPECTED_HV" != "$FIXED_HARNESS_EXPECTED" ]]; then
  echo "ERROR: expected-fixed harness $EXPECTED_HV != pinned $FIXED_HARNESS_EXPECTED" | tee -a "$LOG"
  exit 2
fi
# Ensure immutable-run-manifest exists (bootstraps from expected-fixed when needed).
python3 - <<PY
import sys
from pathlib import Path
sys.path.insert(0, "$E2E/cross-product")
from recovery_lib import load_immutable_run_manifest, compute_harness_version, write_run_abort, compare_to_immutable
run_dir = Path("$RESULTS")
imm = load_immutable_run_manifest(run_dir)
live = compute_harness_version(root=Path("$ROOT"), commit=__import__("subprocess").check_output(["git","-C","$ROOT","rev-parse","HEAD"], text=True).strip())
if imm:
    mismatches = compare_to_immutable(imm, live)
    if mismatches:
        write_run_abort(run_dir, abort_reason="HARNESS_DRIFT", expected=imm, actual=live, detail={"mismatches": mismatches, "phase": "orchestrator_start"})
        print("HARNESS_DRIFT at orchestrator start:", mismatches)
        raise SystemExit(42)
print("immutable_ok", imm.get("harness_version") if imm else None)
PY
ORCH_BOOT_RC=$?
if [[ "$ORCH_BOOT_RC" -eq 42 ]]; then
  python3 - <<PY
...
doc["phase"]="aborted_harness_drift"
doc["updated_at"]=datetime.datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%SZ")
doc["abort_reason"]="HARNESS_DRIFT"
json.dump(doc, open(p,"w"), indent=2)
PY
  echo "ERROR: HARNESS_DRIFT — refusing to resume under drifted harness" | tee -a "$LOG"
  exit 42
fi
...
Run: $SOURCE_RUN_ID"
  ...
  # Quarantine wrong-harness artifacts under .bad-* without deleting prior quarantine evidence.
  python3 - <<PY
import json
from pathlib import Path
...
for art in sorted(base.glob("xp-normal-*-ROUTE_ON")):
  ...
  if hv and hv!=exp:
    ...
    if bad.exists():
      archived=art.parent/f".bad-{art.name}-{ts}"
      bad.rename(archived)
      print("archived prior quarantine", bad.name, "->", archived.name)
    art.rename(bad)
    print("quarantined", art.name, "->", bad.name)
PY
  # Refuse resume when live harness != expected (must use fixed worktree).
  LIVE_HV="$(npx tsx cross-product/harness-version.ts | python3 -c "import json,sys;print(json.load(sys.stdin)['harness_version'])")"
  if [[ "$LIVE_HV" != "$EXPECTED_HV" ]]; then
    log "ERROR: refuse resume_route_on — live harness drifted (live=$LIVE_HV expected=$EXPECTED_HV)"
    python3 - <<PY
import sys
from pathlib import Path
sys.path.insert(0, "$E2E/cross-product")
from recovery_lib import write_run_abort, load_immutable_run_manifest
imm = load_immutable_run_manifest(Path("$RESULTS")) or {}
write_run_abort(
  Path("$RESULTS"),
  abort_reason="HARNESS_DRIFT",
  expected={"git_commit": imm.get("git_commit"), "harness_version": "$EXPECTED_HV"},
  actual={"harness_version": "$LIVE_HV"},
  detail={"phase": "resume_route_on_refused"},
)
PY
    save_state aborted_harness_drift
    notify ERROR "Type: HARNESS_DRIFT — resume refused
Run: $SOURCE_RUN_ID
Live: ${LIVE_HV:0:16}...
    ...
    wait_route_on)
      ...
      if [[ -f "$ORCH_PID_FILE" ]]; then
        ...
        if ! kill -0 "$ORCH_PID" 2>/dev/null; then
          CHILD_DEAD=1
          log "route-on orchestrator pid $ORCH_PID exited"
        fi
      else
        CHILD_DEAD=1
      fi
      # Abort file or live harness drift → terminal ABORTED (no resume storm).
      if [[ -f "$RESULTS/run-abort.json" ]]; then
        log "run-abort.json present — entering aborted_harness_drift"
        save_state aborted_harness_drift
        continue
      fi
      LIVE_HV="$(npx tsx cross-product/harness-version.ts | python3 -c "import json,sys;print(json.load(sys.stdin)['harness_version'])")"
      if [[ "$LIVE_HV" != "$EXPECTED_HV" ]]; then
        log "HARNESS_DRIFT detected in wait_route_on — aborting (no shard resume)"
        python3 - <<PY
import sys
from pathlib import Path
sys.path.insert(0, "$E2E/cross-product")
...
write_run_abort(
  ...
  expected={"git_commit": imm.get("git_commit"), "harness_version": "$EXPECTED_HV"},
  actual={"harness_version": "$LIVE_HV"},
  detail={"phase": "wait_route_on", "child_dead": bool($CHILD_DEAD)},
)
PY
        stop_route_on_orchestrator
        save_state aborted_harness_drift
        notify ERROR "Type: ABORTED — HARNESS_DRIFT / STALE_LOCK
Run: $SOURCE_RUN_ID
Live: ${LIVE_HV:0:16}...
Expected: ${EXPECTED_HV:0:16}..."
        continue
...
PY
      ...
      set -e
      if [[ $HV_RC -eq 2 ]]; then
        notify ERROR "Type: shard harness_version mismatch vs fixed harness — abort (no auto-resume under uncertainty)
Run: $SOURCE_RUN_ID"
        stop_route_on_orchestrator
        python3 - <<PY
import sys
...
write_run_abort(
  Path("$RESULTS"),
  abort_reason="HARNESS_DRIFT",
  expected={"harness_version": "$EXPECTED_HV", "git_commit": imm.get("git_commit")},
  actual={"harness_version": "shard_row_mismatch"},
  detail={"phase": "wait_route_on_shard_mismatch"},
)
PY
        save_state aborted_harness_drift
        continue
      fi
      if [[ "$CHILD_DEAD" -eq 1 ]] && [[ -f "$RESULTS/run-metadata.json" ]]; then
        # Child finished; if incomplete and not done, do not spin forever — surface status.
        if ! route_on_done; then
          FAILED_SHARDS="$(python3 -c "import json;print(json.load(open('$RESULTS/run-metadata.json')).get('failed_shards',0))" 2>/dev/null || echo 0)"
          if [[ "${FAILED_SHARDS}" != "0" ]]; then
            log "route-on child dead with failed_shards=$FAILED_SHARDS — check for abort/drift before resume"
          fi
        fi
      fi
      if route_on_done; then
    ...
    aborted_harness_drift)
      log "Terminal phase aborted_harness_drift — exiting orchestrator (evidence preserved)"
      notify ERROR "Type: ABORTED — HARNESS_DRIFT
Run: $SOURCE_RUN_ID
Status: Orchestrator stopped; use recovery-attempt plan + fixed worktree to resume"
      exit 42
      ;;
...
if sup.exists():
  ...
  rows_path=Path("$REPORTS_ROOT/$RERUN_ID/xp-normal-000-ROUTE_ON/cross-product-results.jsonl")
  if rows_path.exists():
...
rerun=Path("$REPORTS_ROOT/$RERUN_ID")
if rerun.exists():
  os.symlink(rerun, Path("$MERGE_ROOT/inputs/shard0_rerun"))
off=Path("$REPORTS_ROOT/$OFF_ID")
if off.exists():
