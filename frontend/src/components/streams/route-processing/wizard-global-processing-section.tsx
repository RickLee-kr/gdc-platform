...
export function WizardGlobalProcessingSection({
  state,
  editing,
  onEditToggle,
  children,
  ...
  const cards = buildGlobalCards(state)

  return (
    <section className="space-y-3" data-testid="global-processing-section">
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div>
          ...
          <p className="mt-0.5 text-[11px] text-slate-600 dark:text-gdc-muted">
            These settings are applied to all routes by default.
          </p>
        </div>
        <button
          type="button"
          ...
          data-testid="global-processing-edit-toggle"
        >
          {editing ? 'Close Global Settings' : 'Edit Global Settings'}
        </button>
      </div>
        ...
        {cards.map((card) => {
          ...
          return (
            <article
              ...
              data-testid={`global-processing-card-${card.key}`}
            >
      ...
      <p className="rounded-md border border-violet-200/70 bg-violet-500/[0.06] px-3 py-2 text-[11px] text-violet-900 dark:border-violet-500/30 dark:text-violet-100">
        Routes will inherit these settings unless overridden.
      </p>

      {editing ? (
        <div className="space-y-4 rounded-lg border border-slate-200/90 bg-slate-50/50 p-3 dark:border-gdc-border dark:bg-gdc-section/40" data-testid="global-processing-editor">
          {children}
        </div>
      ) : null}
    </section>
...
export function summarizeGlobalProtection(dataProtection: WizardDataProtectionState): string {
