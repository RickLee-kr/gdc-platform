...
export function ValidationDeliveryPage() {
  ...
  const filtered = (rows ?? []).filter((r) => r.validation_type === 'FULL_RUNTIME')

  return (
    <div className="space-y-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-2 text-slate-600 dark:text-gdc-mutedStrong">
          <RadioTower className="h-4 w-4 text-violet-600 dark:text-violet-400" aria-hidden />
          <span className="text-xs font-medium uppercase tracking-wide">Delivery validation (FULL_RUNTIME)</span>
        </div>
        <button
          type="button"
          onClick={() => void load()}
        ...
        <table className={opTable}>
          <thead>
            <tr className={opThRow}>
              ...
              <th className={opTh}>Checkpoint expect</th>
          ...
          <tbody>
            ...
            {!loading && filtered.length === 0 ? (
              <tr className={cn(opTr, opStateRow)}>
                <td className={opTd} colSpan={5}>
                  No FULL_RUNTIME validations. These assert route_send_success and run_complete in delivery_logs.
                </td>
              </tr>
            ) : null}
            {filtered.map((r) => (
              <tr key={r.id} className={opTr}>
                ...
                <td className={opTd}>{r.expect_checkpoint_advance ? 'advance' : 'skip'}</td>
                <td className={opTd}>
                ...
                <td className={opTd}>
                  <span className="line-clamp-2 font-mono text-[10px]">{r.last_error ?? '—'}</span>
                </td>
              </tr>
            ))}
          </tbody>
