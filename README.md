...
**Docker로 띄울 때** 플랫폼 스택(`docker-compose.platform.yml`, DB 이름 `gdc`)과 **개발 검증 랩**(`./scripts/validation-lab/start.sh`, DB `gdc_test`, WireMock 등)은 서로 다른 절차입니다. 플랫폼만 올리면 `[DEV VALIDATION]` 커넥터는 나오지 않는 것이 정상입니다. 비교·트러블슈팅은 영문 가이드 [`docs/local-docker-workflow.md`](docs/local-docker-workflow.md)를 보세요.
...
**pytest / Alembic / isolated test DB** (host port **55432**, database **`gdc_test`**)는 `docker-compose.test.yml`의 `postgres-test`와 동일한 URL을 사용합니다. 예: `TEST_DATABASE_URL` / `DATABASE_URL` = `postgresql://gdc:gdc@127.0.0.1:55432/gdc_test`. Backfill Phase 2 스모크는 `./scripts/test/run-backfill-tests.sh`를 참고하세요. 상세는 영문 [`docs/testing/backfill-phase2-pytest.md`](docs/testing/backfill-phase2-pytest.md).
...
# 1-B) 또는 pytest 전용 격리 DB (호스트 포트 55432, DB 이름 gdc_test)
