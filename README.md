...
**pytest (host)** must not use the same **`gdc`** catalog as the running API. Use the dedicated **`gdc_pytest`** catalog (`docker-compose.test.yml` `postgres-test`, default port **55441**) as `TEST_DATABASE_URL`. Full-suite helper: `bash scripts/test/run-backend-full.sh`. Backfill Phase 2 smoke: `./scripts/test/run-backfill-tests.sh`. Details: [`docs/testing/backfill-phase2-pytest.md`](docs/testing/backfill-phase2-pytest.md), [`docs/testing/backend-full-test.md`](docs/testing/backend-full-test.md).

```bash
# 1-A) Full development platform (API/frontend/proxy/PostgreSQL/runtime validation)
./scripts/dev/start-platform.sh
export DATABASE_URL=postgresql://gdc:gdc@127.0.0.1:55432/gdc

# 1-B) Docker test stack (host pytest must still use gdc_pytest, not gdc)
docker compose -f docker-compose.test.yml --profile test up -d postgres-test
export DATABASE_URL=postgresql://gdc:gdc@127.0.0.1:55441/gdc
