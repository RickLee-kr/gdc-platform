...
def test_get_enabled_routes_by_stream_id_filters_enabled_and_stream_id() -> None:
    db = _make_db()
    ...
    db.add_all(
        [
            ...
            Route(stream_id=stream1.id, destination_id=dst.id, enabled=False, failure_policy="LOG_AND_CONTINUE", formatter_config_json={}, rate_limit_json={}, status="DISABLED"),
...
def test_load_stream_context_success() -> None:
    db = _make_db()
    connector = Connector(name="c1", description=None, status="RUNNING")
    db.add(connector)
    db.flush()
...
def test_load_stream_context_requires_enabled_routes() -> None:
    db = _make_db()
    connector = Connector(name="c1", description=None, status="RUNNING")
    db.add(connector)
    db.flush()
