...
def test_observe_extracted_event_schema_uses_short_db_write_without_active_db() -> None:
    """Schema observation opens a short write session; it does not require ``_active_db``."""

    runner = StreamRunner()
    runner._active_db = None
    with patch.object(StreamRunner, "_db_write", wraps=StreamRunner._db_write) as db_write:
