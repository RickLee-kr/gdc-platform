...
def test_runtime_observability_filters_and_cursor_edges(
    runtime_ui_smoke_client: TestClient, db_session: Session
) -> None:
    h = _seed_stream_two_routes(db_session)
    sid = h["stream_id"]
    ...
    # logs/search: combined filters intersection
    search = runtime_ui_smoke_client.get(
    ...
    rows = search.json()["logs"]
    assert len(rows) == 1
    assert rows[0]["message"] == "match-a"

    # logs/page: cursor_created_at + cursor_id should advance with no overlap
    p1 = runtime_ui_smoke_client.get(
        "/api/v1/runtime/logs/page",
        params={"stream_id": sid, "limit": 1},
    )
    assert p1.status_code == 200
    b1 = p1.json()
    assert b1["total_returned"] == 1
    assert b1["next_cursor_created_at"] is not None
    assert b1["next_cursor_id"] is not None

    p2 = runtime_ui_smoke_client.get(
        "/api/v1/runtime/logs/page",
        params={
            "stream_id": sid,
            "limit": 2,
            "cursor_created_at": b1["next_cursor_created_at"],
            "cursor_id": b1["next_cursor_id"],
        },
    )
    assert p2.status_code == 200
    ids1 = {item["id"] for item in b1["items"]}
    ids2 = {item["id"] for item in p2.json()["items"]}
    assert ids1.isdisjoint(ids2)

    # timeline/failure-trend: limit + filter params should be respected
    tl = runtime_ui_smoke_client.get(
        f"/api/v1/runtime/timeline/stream/{sid}",
        params={"route_id": rid_a, "destination_id": did_a, "limit": 1},
    )
    assert tl.status_code == 200
    tl_body = tl.json()
    assert tl_body["total"] == 1
    assert len(tl_body["items"]) == 1

    ft = runtime_ui_smoke_client.get(
        "/api/v1/runtime/failures/trend",
        params={"stream_id": sid, "route_id": rid_a, "destination_id": did_a, "limit": 1},
    )
    assert ft.status_code == 200
    ft_body = ft.json()
    assert ft_body["total"] <= 1
    assert len(ft_body["buckets"]) <= 1
