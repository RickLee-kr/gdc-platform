...
def reset_wiremock_request_journal(*, base_url: str | None = None) -> bool:
    ...
    try:
        with httpx.Client(timeout=5.0) as client:
            # Platform lab WireMock uses --no-request-journal; treat as already clear.
            probe = client.get(f"{base}/__admin/requests")
            if _journal_disabled_response(status_code=probe.status_code, body=probe.text or ""):
                logger.info("%s", {"stage": "wiremock_journal_reset", "ok": True, "status_code": probe.status_code, "base": base, "reason": "journal_disabled"})
                return True
            if probe.status_code == 200:
                payload = probe.json()
                if isinstance(payload, dict) and payload.get("requestJournalDisabled") is True:
                    logger.info("%s", {"stage": "wiremock_journal_reset", "ok": True, "status_code": 200, "base": base, "reason": "journal_disabled"})
                    return True
